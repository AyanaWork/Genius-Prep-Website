...
// Debounce hook — keeps API calls calm while the user types.
...
function BrowseTutors() {
  ...
  // Module-code autocomplete data, fetched once.
  ...
  useEffect(() => {
    // Load module-code list once for autocomplete.
  ...
  useEffect(() => {
    ...
    setPage(1);
  }, [filters.subject, filters.availabilityStatus, filters.sort, debouncedSearch]);
  ...
  const loadTutors = async () => {
    try {
      ...
      // Only attach search term when there's something to search.
  ...
  // Filter the autocomplete list against current input. Done client-side
  // for instant feedback — we already loaded the list once.
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          ...
          <div className="relative mb-4" ref={searchBoxRef}>
            <label className="block text-sm font-semibold text-white/80 mb-2">
              Search by module code, subject, or tutor name
            </label>
            <div className="relative">
              <input
                ...
                className="w-full pl-11 pr-10 py-3 bg-[#1e293b] border border-[#334155] rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-500 uppercase"
            ...
            {/* Module-code autocomplete suggestions */}
            {showSuggestions && codeSuggestions.length > 0 && (
              <div className="absolute z-20 mt-1 w-full bg-[#1e293b] border border-[#334155] rounded-xl shadow-xl overflow-hidden">
                <div className="px-4 py-2 text-xs text-gray-400 border-b border-[#334155]">
                  Module codes
                </div>
                {codeSuggestions.map((code) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2 hover:bg-[#00CC99]/10 transition text-white"
                  >
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-r from-[#00CC99]/10 to-emerald-500/5 border border-[#00CC99]/20 rounded-2xl px-6 py-4">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Tell us what you're looking for and our team will hand-pick a shortlist.
            </p>
          </div>
          <button
            onClick={() => navigate('/request-tutor')}
            className="px-6 py-3 bg-[#00CC99] text-[#0f172a] rounded-xl font-bold hover:scale-105 transition whitespace-nowrap"
          >
            Request a Tutor →
          </button>
        </div>
        ...
        {!loading && tutors.length === 0 && (
          <div className="glass-card rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-400 mb-6">
              Try a different module code, subject, or clear your filters.
            </p>
            <button
              onClick={() => navigate('/request-tutor')}
              className="px-6 py-3 bg-[#00CC99] text-[#0f172a] rounded-xl font-bold"
            >
              Request a Tutor instead
            </button>
          </div>
        )}
            ...
            {tutors.map((tutor) => (
              ...
              >
                ...
                <div className="p-6 pt-4">
                  ...
                  {/* Module codes — surface what students actually search for */}
                  {tutor.module_codes && tutor.module_codes.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3 justify-center">
                      {tutor.module_codes.slice(0, 3).map((code) => (
                        <span
                          key={code}
                          className="px-2 py-0.5 bg-yellow-500/10 text-yellow-300 rounded-md text-xs font-mono font-semibold border border-yellow-500/20"
                        >
                          {code}
                        </span>
                      ))}
        ...
        {!loading && totalPages > 1 && (
          <div className="mt-10 flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 rounded-lg glass-card text-white disabled:opacity-30 disabled:cursor-not-allowed"
            >
              ← Prev
            </button>
