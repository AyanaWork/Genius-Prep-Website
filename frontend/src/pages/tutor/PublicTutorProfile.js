...
function PublicTutorProfile() {
  ...
  const isOwnProfile = currentUser?.role === 'tutor' && currentUser?.tutorProfileId === parseInt(id);
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-400">Loading tutor profile...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/')} className="px-6 py-3 bg-[#00CC99] text-[#0f172a] rounded-xl font-bold">
            Back to Home
          </button>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="glass-card rounded-3xl p-6 sticky top-24">
              {showBookingForm ? (
                // --- BOOKING FORM VIEW (toggled when student clicks "Request Tutor") ---
                <>
                  <div className="text-center mb-6">
                    {tutor.profile_picture_url ? (
                      <img
                        ...
                        className="w-24 h-24 rounded-full mx-auto object-cover border-4 border-[#00CC99]"
                      />
                    ) : (
                      <div className="w-24 h-24 rounded-full mx-auto bg-[#00CC99]/20 flex items-center justify-center border-2 border-[#00CC99]">
                        <span className="text-3xl font-bold text-[#00CC99]">
                ...
                // --- NORMAL TUTOR SUMMARY (initial view) ---
                <>
                  <div className="text-center mb-6">
                    {tutor.profile_picture_url ? (
                      <img
                        src={tutor.profile_picture_url}
                        alt={tutor.display_name}
                        className="w-32 h-32 rounded-full mx-auto object-cover border-4 border-[#00CC99]"
                      />
                    ) : (
                      <div className="w-32 h-32 rounded-full mx-auto bg-[#00CC99]/20 flex items-center justify-center border-2 border-[#00CC99]">
                        <span className="text-4xl font-bold text-[#00CC99]">
                          {tutor.display_name?.charAt(0).toUpperCase()}
                        </span>
                      </div>
                    )}
                    {tutor.is_elite && (
                      <div className="inline-flex items-center gap-2 mt-3 px-3 py-1 bg-yellow-500/20 text-yellow-400 rounded-full text-sm">
                        ⭐ Elite Tutor
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-[#0f172a]/5 rounded-xl p-3 text-center">
                      <div className="text-2xl font-bold text-[#00CC99]">{tutor.years_experience || 0}</div>
                      <div className="text-xs text-gray-400">Years Exp.</div>
                    </div>
                    <div className="bg-[#0f172a]/5 rounded-xl p-3 text-center">
                      <div className="text-2xl font-bold text-[#00CC99]">R{tutor.hourly_rate || 0}</div>
                  ...
                  {tutor.availability_status !== 'active' && (
                    <p className="text-sm text-gray-400 text-center mt-2">Currently unavailable</p>
                  )}
                  {isStudent && canReview && !showReviewForm && (
                    <button
                      onClick={() => setShowReviewForm(true)}
                      className="w-full mt-4 py-3 glass-card rounded-xl font-semibold hover:border-[#00CC99]/50 transition"
                    >
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="glass-card rounded-3xl p-8">
              ...
              <p className="text-gray-300 whitespace-pre-line">{tutor.bio || 'No bio available.'}</p>
            </div>
            ...
            <div className="glass-card rounded-3xl p-8">
              ...
              <div className="flex flex-wrap gap-2">
                {tutor.subjects?.map((s, i) => (
                  <span key={i} className="px-4 py-2 bg-[#00CC99]/10 rounded-lg text-sm border border-[#00CC99]/30">
                    {s}
                  </span>
                ))}
              </div>
            ...
            {tutor.module_codes?.length > 0 && (
              <div className="glass-card rounded-3xl p-8">
                <h2 className="text-2xl font-bold mb-4">University Modules</h2>
                <div className="flex flex-wrap gap-2">
                  {tutor.module_codes.map((c, i) => (
                    <span key={i} className="px-3 py-1 bg-blue-500/20 rounded-md font-mono text-sm">
                      {c}
                    </span>
                  ))}
                </div>
            ...
            {(tutor.teaching_mode || tutor.location) && (
              <div className="glass-card rounded-3xl p-8">
                ...
                <div className="space-y-3">
                  {tutor.teaching_mode && (
                    <div>
                      <span className="text-2xl mr-2">💻</span> {tutor.teaching_mode}
                    </div>
                  )}
                  {tutor.location && (
                    <div>
                      <span className="text-2xl mr-2">📍</span> {tutor.location}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="glass-card rounded-3xl p-8">
              ...
              {reviewsLoading ? (
                ...
                <div className="text-center py-8 text-gray-400">No reviews yet. Be the first!</div>
              ) : (
