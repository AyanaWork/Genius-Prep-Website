          ...
          <div className="glass-card rounded-3xl p-8 md:p-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input type="text" name="displayName" value={formData.displayName} onChange={handleChange} className="w-full px-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" required />
              </div>

              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Bio</label>
                <textarea name="bio" value={formData.bio} onChange={handleChange} rows="5" className="w-full px-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" placeholder="Tell students about your teaching style..." />
              </div>

              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Qualifications</label>
                <input type="text" name="qualifications" value={formData.qualifications} onChange={handleChange} className="w-full px-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" placeholder="BSc Computer Science, TEFL..." />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {SUBJECT_OPTIONS.map(sub => (
                    <button type="button" key={sub} onClick={() => handleSubjectToggle(sub)} className={`px-3 py-2 rounded-lg text-sm transition ${formData.subjects.includes(sub) ? 'bg-[#00CC99] text-[#0f172a] font-semibold' : 'bg-[#1e2a3a] text-white hover:bg-[#00CC99]/20'}`}>{sub}</button>
                  ))}
                </div>
                {showCustomInput ? (
                  <div className="flex gap-2 mt-3">
                    <input type="text" value={customSubject} onChange={e => setCustomSubject(e.target.value)} className="flex-1 px-4 py-2 bg-[#1e2a3a] border border-white/10 rounded-lg text-white placeholder-gray-400" placeholder="Subject name" />
              ...
              <div>
                ...
                <input type="text" name="moduleCodes" value={formData.moduleCodes} onChange={handleChange} className="w-full px-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" placeholder="MAT101, PHY201" />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" name="hourlyRate" value={formData.hourlyRate} onChange={handleChange} className="w-full pl-8 pr-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" placeholder="150" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2 text-white">Years Experience</label>
                  <input type="number" name="yearsExperience" value={formData.yearsExperience} onChange={handleChange} className="w-full px-4 py-3 bg-[#1e2a3a] border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white placeholder-gray-400" placeholder="3" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {TEACHING_MODES.map(mode => (
                    <button type="button" key={mode} onClick={() => setFormData(prev => ({ ...prev, teachingMode: mode }))} className={`px-6 py-2 rounded-xl capitalize transition ${formData.teachingMode === mode ? 'bg-[#00CC99] text-[#0f172a] font-bold' : 'bg-[#1e2a3a] text-white hover:bg-[#00CC99]/20'}`}>{mode}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/30 p-6 rounded-xl">
                ...
                <div className="mb-4">
                  ...
                  <input type="file" accept=".pdf,.jpg,.png" onChange={e => setIdDocument(e.target.files[0])} className="w-full text-sm text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:bg-[#00CC99]/20 file:text-[#00CC99] file:border-0 file:cursor-pointer" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-white">Academic Transcript *</label>
                  <input type="file" accept=".pdf,.jpg,.png" onChange={e => setTranscript(e.target.files[0])} className="w-full text-sm text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:bg-[#00CC99]/20 file:text-[#00CC99] file:border-0 file:cursor-pointer" />
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button type="button" onClick={() => navigate('/tutor/dashboard')} className="px-6 py-3 glass-card rounded-xl text-white hover:border-[#00CC99]/50">Cancel</button>
              </div>
            </form>
          </div>
        </div>
