...
function RequestTutor() {
  ...
  const [data, setData] = useState({
    ...
    institution: '',
    subjects: [],
    moduleCodesRaw: '',           // comma-separated input -> array on submit
    budgetPerHour: '',
  ...
  // Per-step validation. Returns error string or null.
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      const moduleCodes = data.moduleCodesRaw
        .split(',')
        .map((c) => c.trim())
        .filter(Boolean);
      ...
      <div className="min-h-[calc(100vh-6rem)] flex items-center justify-center px-4 pt-24 pb-12">
        <div className="max-w-md w-full glass-card rounded-3xl p-10 text-center">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={() => navigate('/')}
              className="w-full py-3 glass-card rounded-xl text-white"
            >
              Back to home
            </button>
          </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {REQUESTER_TYPES.map((t) => (
                    <button
                      key={t.value}
                      type="button"
                      onClick={() => update('requesterType', t.value)}
                      className={`py-4 rounded-xl text-center transition ${
                        data.requesterType === t.value
                          ? 'bg-[#00CC99] text-[#0f172a] font-bold'
                          : 'bg-[#0f172a]/40 hover:bg-[#0f172a]/60 text-white'
                      }`}
                    >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                  />
                </div>
                <div>
              ...
              {data.requesterType === 'bursary' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                    placeholder="e.g. Acme Bursary Trust"
                  />
                </div>
              )}
            </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={data.moduleCodesRaw}
                  onChange={(e) => update('moduleCodesRaw', e.target.value)}
                  className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                  placeholder="FRK300, MAT101, STK220"
                />
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {data.subjects.map((s) => (
                      <span key={s} className="px-3 py-1 bg-[#00CC99]/20 rounded-full text-sm flex items-center gap-2">
                        {s}
                        <button type="button" onClick={() => toggleSubject(s)} className="text-[#00CC99]">×</button>
                      </span>
                    ))}
                  </div>
                )}
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-sm transition ${
                        data.subjects.includes(s)
                          ...
                          : 'bg-[#0f172a]/40 text-white hover:bg-[#00CC99]/20'
                      }`}
                    >
                      {s}
                    </button>
            ...
            <div className="space-y-5">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={data.budgetPerHour}
                    onChange={(e) => update('budgetPerHour', e.target.value)}
                    className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                    placeholder="e.g. 250"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => update('numberOfStudents', e.target.value)}
                    className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                  />
                </div>
              </div>
                ...
                <div className="flex gap-3">
                  {FORMATS.map((f) => (
                    <button
                      key={f.value}
                      type="button"
                      onClick={() => update('preferredFormat', f.value)}
                      className={`flex-1 py-3 rounded-xl transition ${
                        data.preferredFormat === f.value
                          ? 'bg-[#00CC99] text-[#0f172a] font-bold'
                          : 'bg-[#0f172a]/40 text-white hover:bg-[#00CC99]/20'
                      }`}
                    >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-[#0f172a]/40 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white"
                  placeholder="Schedule preferences, learning goals, exam dates..."
                />
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500 mt-4">
                By submitting you agree that the Genius Prep admin team can contact you on the email
                and phone number above. Your contact details are never published.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3 mt-8">
            {step > 1 && (
              <button
                type="button"
                onClick={back}
                className="px-6 py-3 glass-card rounded-xl text-white"
                disabled={submitting}
              >
                ← Back
              </button>
            ...
            {step < 4 && (
              <button
                type="button"
                onClick={next}
                className="flex-1 py-3 bg-[#00CC99] text-[#0f172a] rounded-xl font-bold hover:scale-105 transition"
              >
                Continue →
              </button>
            )}
