      ...
      <div className="pt-24 pb-16 px-6 container mx-auto">
        <div className="max-w-4xl mx-auto">
          <div className="glass-card rounded-3xl p-8 md:p-12">
            <h1 className="text-3xl md:text-4xl font-black mb-2 text-center leading-[1.15] pb-2 bg-gradient-to-r from-[#00CC99] to-emerald-400 bg-clip-text text-transparent">Your Student Profile</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input type="text" name="displayName" value={formData.displayName} onChange={handleChange} className="w-full px-4 py-3 bg-[#0f172a]/5 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white" required />
              </div>

              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Education Level</label>
                  <select name="educationLevel" value={formData.educationLevel} onChange={handleChange} className="w-full px-4 py-3 bg-[#0f172a]/5 border border-white/10 rounded-xl focus:border-[#00CC99] focus:outline-none text-white">
                ...
                {formData.subjectsInterested.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {formData.subjectsInterested.map(s => (
                      <span key={s} className="px-3 py-1 bg-[#00CC99]/20 rounded-full text-sm flex items-center gap-2 text-white">{s}<button type="button" onClick={() => handleRemoveSubject(s)} className="text-[#00CC99]">×</button></span>
                    ))}
                  </div>
                )}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {SUBJECT_OPTIONS.map(sub => (
                    <button type="button" key={sub} onClick={() => handleSubjectToggle(sub)} className={`px-3 py-2 rounded-lg text-sm transition ${formData.subjectsInterested.includes(sub) ? 'bg-[#00CC99] text-[#0f172a]' : 'bg-[#0f172a]/5 text-white hover:bg-[#00CC99]/20'}`}>{sub}</button>
                  ))}
                </div>
                {showCustomInput ? (
                  <div className="flex gap-2 mt-3">
                    <input type="text" value={customSubject} onChange={e => setCustomSubject(e.target.value)} className="flex-1 px-4 py-2 bg-[#0f172a]/5 border border-white/10 rounded-lg text-white placeholder-gray-400" placeholder="Subject name" />
                    ...
                    <button type="button" onClick={() => setShowCustomInput(false)} className="px-4 py-2 bg-[#0f172a]/10 rounded-lg text-white">Cancel</button>
                  </div>
                ) : (
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button type="button" onClick={() => navigate('/student/dashboard')} className="px-6 py-3 glass-card rounded-xl text-white hover:border-[#00CC99]/50">Cancel</button>
              </div>
            </form>
          </div>
        </div>
